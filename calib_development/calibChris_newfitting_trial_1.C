...
void drawGausFit(TH2F* inHisto, TGraph& response, TGraph& resolution)
{
   ...
   for(unsigned bin = 2; bin < sampleRangeHigh; )
   {
      ...
      if(ETrueBin.back()->GetEntries() > 5)
	{
	  ...
	  // cccc->SaveAs( ("tmp/"+name+".C").c_str() );
	    ...
	    // respHisto->Fill(ETrue[bin],gausMean[bin]);
	}
   ...
   file1->Close();
   // delete cccc;
 TFile* file2=new TFile("output_1.2_1.55.root","recreate");
  file2->cd();
   ...
   for( unsigned entry = 0; entry < std::min((unsigned)50000000,(unsigned)(tree->GetEntriesFast()) ); entry++)
     {
       ...
       // if (fabs(eta_) < 1.20 || fabs(eta_)>1.55) continue;
       if (fabs(eta_) < 1.2 || fabs(eta_)> 1.55) continue;    
       // if (true_< 60 || true_ > 70   ) continue;
       if(tree->GetBranchStatus("true"))
...
// TGraph *response = new TGraph();
// TGraph *resolution = new TGraph();
// TGraph *responseRaw = new TGraph();
// TGraph *resolutionRaw = new TGraph();
// TGraph *responseCor = new TGraph();
// TGraph *resolutionCor = new TGraph();
// TGraph *responseEta = new TGraph();
// TGraph *resolutionEta = new TGraph();
// TGraph *responseEtaEtaEH = new TGraph();
// TGraph *responseEtaHCorrEtaEH = new TGraph();
// TGraph *responseEtaEtaH = new TGraph();
// TGraph *responseEtaHCorrEtaH = new TGraph();
...
void calibChris() 
{
   ...
   */
   ...
   if(freezeparameters) {
     ...
     //fbEtaEndcapEH
     // functionEndcapBetaEcalHcal->SetParameter(0,-0.0192991);
     // functionEndcapBetaEcalHcal->SetParameter(1,-0.265);
     // functionEndcapBetaEcalHcal->SetParameter(2,80.5502);
     // functionEndcapBetaEcalHcal->SetParameter(3,100.1104);
     // functionEndcapBetaEcalHcal->SetParameter(4,-10);
